...
async def start(client, message):
        ...
        if channels:  
            ...
            if btn:
                ...
                caption = (
                    ...
                    "🛑 ʏᴏᴜ ᴍᴜsᴛ ᴊᴏɪɴ ᴛʜᴇ ʀᴇǫᴜɪʀᴇᴅ ᴄʜᴀɴɴᴇʟs ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.👉 ᴊᴏɪɴ ᴀʟʟ ᴛʜᴇ ʙᴇʟᴏᴡ ᴄʜᴀɴɴᴇʟs ᴀɴᴅ ᴛʀʏ ᴀɢᴀɪɴ..\n"
                    "🔰ಮುಂದುವರಿಯಲು ಅಗತ್ಯವಿರುವ ಚಾನಲ್‌ಗಳಿಗೆ ನೀವು ಸೇರಬೇಕು. ⏩ಕೆಳಗಿನ ಎಲ್ಲಾ ಚಾನಲ್‌ಗಳಿಗೆ ಸೇರಿ ಮತ್ತು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ."
                )
                await message.reply_photo(
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        if is_valid == True:
            ...
            await message.reply_photo(
                ...
                caption=f"<b>Hey {message.from_user.mention},\n\n𝑌𝑜𝑢 𝐴𝑟𝑒 𝑆𝑢𝑐𝑐𝑒𝑠𝑠𝑓𝑢𝑙 𝑉𝑒𝑟𝑖𝑓𝑖𝑒𝑑 ! 𝑁𝑜𝑤 𝑌𝑜𝑢 𝐻𝑎𝑣𝑒 𝑈𝑛𝑙𝑖𝑚𝑖𝑡𝑒𝑑 𝐴𝑐𝑐𝑒𝑠𝑠 𝐹𝑜𝑟 {DEENDAYAL_VERIFY_EXPIRE} HOURS.\n\n<blockquote>ನಿಮ್ಮನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಪರಿಶೀಲಿಸಲಾಗಿದೆ {DEENDAYAL_VERIFY_EXPIRE}  ಈಗ ನೀವು 1  ಗಂಟೆಯ ಹೊತ್ತಿಗೆ 𝐷𝑖𝑟𝑒𝑐𝑡 ನೀವು 𝑀𝑜𝑣𝑖𝑒  𝐹𝑖𝑙𝑒𝑠  ಪಡೆಯಬಹುದು.</blockquote></b>",
                reply_markup=InlineKeyboardMarkup(btn)
            )
    ...
    if await db.has_premium_access(message.from_user.id):
        ...
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            ...
            l = await message.reply_text(
                text=f"<b>♨️ YOU ARE NOT VERIFIED !\nKINDLY VERIFY TO CONTINUE & YOU CAN GET UNLIMITED ACCESS FOR {DEENDAYAL_VERIFY_EXPIRE} HOURS ✅\n\n<blockquote>🎀 ಈ BOT ನಲ್ಲಿ MOVIE ಪಡೆಯಬೇಕೆಂದರೆ ನೀವು VERIFY ಮಾಡುವುದು ಅವಶ್ಯಕವಾಗಿದೆ, ಒಂದು ಬಾರಿ VERIFY ಆದರ {DEENDAYAL_VERIFY_EXPIRE}  ಗಂಟೆಗಳತನಕ ನೀವು UNLIMITED MOVIES  ಪಡೆಯಬವುದಾಗಿದೆ .</blockquote> \n\n<blockquote>⚡   ಒಂದುವೇಳೆ ನೀವು VERIFY ಮಾಡಲಾಗುವುದಿಲ್ಲ ಎಂದರೆ PREMIUM ತೆಗೆದುಕೊಳ್ಳಬೇಕು , PREMIUM ತೆಗೆದುಕೊಳ್ಳುವುದರಿಂದ ನೀವು UNLIMITED MOVIES ಪಡೆಯಬವುದು ಆಗ VERIFY ಮಾಡುವ ಅಗತ್ಯವಿರುವುದಿಲ್ಲ PLAN DETAILS ಗಳಿಗಾಗಿ CLICK ಮಾಡಿ /plan</blockquote></b>",
                protect_content=False,
                reply_markup=InlineKeyboardMarkup(btn)
            )
            await asyncio.sleep(180)
